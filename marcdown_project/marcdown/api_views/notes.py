...
class NoteViewSet(viewsets.ViewSet):
    def retrieve(self, request, pk=None):
        ...
        if user.is_authenticated:
            ...
            if note.allow_reading_by_user(profile):
                data = {
                    'is_owner' : profile == note.owner,
                    'can_write' : note.allow_update_from_user(profile),
                }
            else:
    ...
    def update(self, request, pk=None):
        ...
        Only the owner has the right to change those settings
        '''
        user = request.user
        if user.is_authenticated:
            data = request.data
            ...
            if note.owner == user.profile:
                ...
                return JsonResponse(status=status.HTTP_403_FORBIDDEN, data={"status" : False, "message" : "You are not allowed to edit this note's permissions"})
        else:
            return JsonResponse(status=status.HTTP_401_UNAUTHORIZED, data={"status" : False, "message" : "Authentication is required"})
    ...
    def destroy(self, request, pk=None):
        ...
        Only the owner has the right to delete a note
        '''
        user = request.user
        if user.is_authenticated:
            queryset = Note.objects.all()
            ...
            if user.profile == note.owner:
                ...
                return JsonResponse(status=status.HTTP_403_FORBIDDEN, data={"status" : False, "message" : "You are not allowed to delete this note"})
        else:
            return JsonResponse(status=status.HTTP_401_UNAUTHORIZED, data={"status" : False, "message" : "Authentication is required"})